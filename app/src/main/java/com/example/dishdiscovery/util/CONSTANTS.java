package com.example.dishdiscovery.util;

public class CONSTANTS {
    public static final String MEAL_OF_THE_DAY = "MEAL_OF_THE_DAY";
    public static final String SHARED_PREFS = "SHARED_PREFS";
    public static final String CATEGORY_NAME = "CATEGORY_NAME";
    public static final String MEAL_ID = "MEAL_ID";
    public static final String MEAL_AREA = "MEAL_AREA";
    public static final String MEAL_CATEGORY = "MEAL_CATEGORY";
    public static final String USER_ID = "USER_ID";
    public static final String USERS_REALTIME_DB = "users";
    public static final String FAVORITES_REALTIME_DB = "favorites";
    public static final String FAVMEALS = "favMeals";
    public static final String WEEKLY_MEALS_REALTIME_DB = "weekly";
    public static final String WEEKLY_MEALS ="weeklyMeals"; ;
}
